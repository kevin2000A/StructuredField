...
TetrahedraStructure::TetrahedraStructure() noexcept
    : device(-1),
      ...
      num_vertices(0),
      num_cells(0),
      num_triangles(0), // 初始化 num_triangles
      gas_buffer_sizes{}, // 初始化 gas_buffer_sizes
...
TetrahedraStructure::TetrahedraStructure(TetrahedraStructure &&other) noexcept
    : device(std::exchange(other.device, -1)),
      ...
      num_triangles(std::exchange(other.num_triangles, 0)), 
      gas_buffer_sizes(std::exchange(other.gas_buffer_sizes, OptixAccelBufferSizes{})), // 添加 gas_buffer_sizes 的移动
...
void TetrahedraStructure::build(const size_t num_vertices,
    ...
    // std::cout << "num_triangles in build(): " << num_triangles << std::endl;
    // We can now free the scratch space buffer used during build and the vertex
    // inputs, since they are not needed by our trivial shading method
    CUDA_CHECK(cudaFree(reinterpret_cast<void *>(d_temp_buffer_gas)));
}

void TetrahedraStructure::update(const float3 *d_updated_vertices) {
    CUDA_CHECK(cudaSetDevice(device));

    // std::cout << "num_triangles in update(): " << num_triangles << std::endl;
    // std::cout << "num_vertices in update(): " << num_vertices << std::endl;


    // 更新顶点数据指针
    this->tetrahedra_vertices = d_updated_vertices;

    if (triangle_indices_ == nullptr) {
    throw std::runtime_error("Error: triangle_indices_ is null in update()");
    }
    if (tetrahedra_vertices == nullptr) {
        throw std::runtime_error("Error: tetrahedra_vertices is null in update()");
    }
    ...
    // 正确设置顶点缓冲区
    CUdeviceptr d_vertices_ptr = reinterpret_cast<CUdeviceptr>(tetrahedra_vertices);
    triangle_input.triangleArray.vertexFormat = OPTIX_VERTEX_FORMAT_FLOAT3;
