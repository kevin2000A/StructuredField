...
struct PyTetrahedraTracer {
   public:
    ...
    void update_tetrahedra(const torch::Tensor &xyz) {
      
        CHECK_FLOAT_DIM3(xyz);
        CHECK_INPUT(xyz);
        CHECK_DEVICE(xyz);
       
        size_t num_vertices = xyz.numel() / 3;
        if (num_vertices != this->tetrahedra_vertices.value().numel() / 3) {
            throw std::runtime_error("The number of vertices must match the original number.");
        }
        
        this->tetrahedra_vertices = xyz;
        
        tracer->update_tetrahedra(
            num_vertices,
            reinterpret_cast<const float3 *>(xyz.data_ptr()));
    }
...
PYBIND11_MODULE(tetranerf_cpp_extension, m) {
    py::class_<PyTetrahedraTracer>(m, "TetrahedraTracer")
        ...
        .def("update_tetrahedra",&PyTetrahedraTracer::update_tetrahedra);
