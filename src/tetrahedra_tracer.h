...
class TetrahedraStructure {
   public:
    ...
    friend void swap(TetrahedraStructure &first, TetrahedraStructure &second) {
        ...
        swap(first.num_triangles, second.num_triangles); 
        swap(first.gas_buffer_sizes, second.gas_buffer_sizes);
    }
    ...
    void update(const float3 *d_updated_vertices);
...
class TetrahedraTracer {
   public:
    ...
    void update_tetrahedra(const size_t num_vertices, const float3 *d_vertices) {
    if (!tetrahedra_structure.defined()) {
        throw std::runtime_error("TetrahedraStructure is not initialized. Please call load_tetrahedra first.");
    }
    tetrahedra_structure.update(d_vertices);
    }
